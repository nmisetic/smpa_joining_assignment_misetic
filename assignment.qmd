...
```{r}
precinct_county_total %>% 
  group_by(precinct_code) %>% 
  filter(office_name=="Governor / Lt. Governor") %>% 
  summarise(total_votes_gov = sum(early_votes, election_night_votes, provisional_votes, mail_in_ballot_1_votes, mail_in_ballot_2_votes)) %>% 
  arrange(desc(total_votes_gov))
...
precinct_county %>% 
  group_by(precinct_code) %>% 
  filter(office_name=="Governor / Lt. Governor") %>% 
  mutate(total_votes_gov = sum(early_votes, election_night_votes, provisional_votes, mail_in_ballot_1_votes, mail_in_ballot_2_votes))
